...
class CarRace extends JFrame implements KeyListener,ActionListener
{
        Connection con=null;
	...
	public CarRace(String title,int id)
	{
    ...
    try {
        String selectQuery = "SELECT User_points FROM Gamification WHERE Userid = ?";
        PreparedStatement selectStatement = con.prepareStatement(selectQuery);
        selectStatement.setInt(1, user_id);
        ResultSet resultSet = selectStatement.executeQuery();

        if (resultSet.next()) {
            int existingScore = resultSet.getInt("User_points");
            int newScore = existingScore + score; // Add the new score to the existing score
            
            String updateQuery = "UPDATE Gamification SET User_points = ? WHERE Userid = ?";
            PreparedStatement updateStatement = con.prepareStatement(updateQuery);
            updateStatement.setInt(1, newScore);
            updateStatement.setInt(2, user_id);
            updateStatement.executeUpdate();
        } else {
            String insertQuery = "INSERT INTO Gamification (Userid, User_points) VALUES (?, ?)";
